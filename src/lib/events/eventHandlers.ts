...
import { pushServiceServer } from "../push/pushServiceServer";
...
async function saveInAppNotification(notifData: {
  ...
  try {
    ...
    // Fallback to localStorage for mock/offline sandbox mode
  ...
  if (recipient.role === "customer" || recipient.role === "both") {
    ...
    switch (eventType) {
      case "AUTH_OTP_REQUESTED":
        // Pre-account verification. Does not have customerId in database.
        // Handled directly inside API route using direct call to emailService.
        break;
